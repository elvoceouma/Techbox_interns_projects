...
class Invoice(models.Model):
    ...
    id_no = fields.Char(related='booking_id.id_no', string="ID Number", store=True, tracking=True)
    ...
    # room_id = fields.Many2many(related='booking_id.room_no', string="Room Number", store=True, tracking=True)
    room_type_id = fields.Many2one(related='booking_id.room_no.room_type_id', string="Room Type", store=True, tracking=True)
    check_in = fields.Datetime(related='booking_id.check_in', string="Check-in Date", store=True, tracking=True)
    check_out = fields.Datetime(related='booking_id.check_out', string="Check-out Date", store=True, tracking=True)
